...
## Professional Companion Upgrade

This update expands Raiders Vault from a basic CRUD planner into a more polished companion-style command platform. The design is inspired by modern game companion hubs that combine guides, tools, interactive map-style planning, item intelligence, overlays, and event alerts into one workflow.
...
- Expanded the Run Planner with a companion intelligence layer that keeps map, condition, target, route, and checklist decisions visible together.
...
- Imported a larger ARC Raiders item catalog, adding 581 item records to the local Item Database while preserving existing inventory counts.
